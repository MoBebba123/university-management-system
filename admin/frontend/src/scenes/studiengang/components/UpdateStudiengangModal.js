...
const AddStudiengangModal = ({ open, handleClose, studiengangToUpdate }) => {
  const alert = useAlert();
  const theme = useTheme();
  const id = studiengangToUpdate.id;
  ...
  useEffect(() => {
    setStudiengangData(studiengangToUpdate);
  }, [studiengangToUpdate, setStudiengangData]);
  ...
  const handelSubmit = (e) => {
    ...
    dispatch(updateStudiengang(id, myForrm));
    setStudiengangData({
      title: "",
      po_version: "",
    });
  ...
  return (
    <>
      ...
      <Modal open={open} style={{}}>
        <Box sx={style}>
          <Header title="UPDATW STUDIENGANG" subtitle="update studiengang" />
